...
export interface ApiCompatibilityChecksOptions {
  skipBaggageChecks?: boolean; // boolean that controls whether or not to verify certain API functionality
  skipInjectExtractChecks?: boolean;
};

/**
 * A function that takes a tracer factory, and tests wheter the initialized tracer fulfills API requirements.
...
function apiCompatibilityChecks(createTracer = () => new Tracer(), options: ApiCompatibilityChecksOptions = {skipBaggageChecks: false, skipInjectExtractChecks: false}): void {
    describe('Tracing API Compatibility', () => {
        let tracer: Tracer;
        let span: Span;
